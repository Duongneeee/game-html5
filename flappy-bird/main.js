...
window.onload = function() {
    const savedUsername = localStorage.getItem("username-flappy-bird");  // Lấy giá trị từ localStorage
    if (savedUsername) {
...
document.querySelector('.login').addEventListener("click",()=>{
    if (!usernameInput.value) {
        ...
        localStorage.setItem("username-flappy-bird", usernameInput.value);
        ...
        gameOver = {
            ...
            w: 226,
            h:160,
            //values for drawing on canvas of badge
            xBadge: cvs.width/2 - 226/2 + 25,
            yBadge: cvs.height/2 - 160 + 88,
            wBadge: 48,
            hBadge: 50,
            //values for drawing on canvas of score
            wScore: 15,
            hScore: 25,
            //object's render function that utilizes all above values to draw image onto canvas
            render: function(resultScore) {
                //only draw this if the game state is on game over
                if (gameState.current == gameState.gameOver) {
                    const localStorageScore = localStorage.getItem('flappy_bird_best_score');
        
                    if(resultScore > localStorageScore){
                        localStorage.setItem('flappy_bird_best_score', resultScore)
                    }
                    ctx.drawImage(theme1, this.imgX,this.imgY,this.width,this.height, this.x,this.y,this.w,this.h)
                    if(resultScore > 3){
                        ctx.drawImage(theme1, this.imgBadgeX,this.imgBadgeY,this.widthBadge,this.heightBadge, this.xBadge,this.yBadge,this.wBadge,this.hBadge)
                    }
        
                    let string = (resultScore).toString()
                    let ones = string.charAt(string.length-1)
                    let tens = string.charAt(string.length-2)
                    let hundreds = string.charAt(string.length-3)
                    //if current score has ones, tens, and hundreds place value only
                    if (resultScore >= 100) {
                        ctx.drawImage(theme2, map[ones].imgX,map[ones].imgY,map[ones].width,map[ones].height, ( this.x + (this.w - 37) ),(this.y + (this.h/2 - 3)),this.wScore,this.hScore)
        
                        ctx.drawImage(theme2, map[tens].imgX,map[tens].imgY,map[tens].width,map[tens].height, ( this.x + (this.w - 37) - this.wScore ),(this.y + (this.h/2 - 3)),this.wScore,this.hScore)
        
                        ctx.drawImage(theme2, map[hundreds].imgX,map[hundreds].imgY,map[hundreds].width,map[hundreds].height, ( this.x + (this.w - 37) - (this.wScore*2)),(this.y + (this.h/2 - 3)),this.wScore,this.hScore)
                    ...
                    if(localStorageScore){
                        let string = (localStorageScore).toString()
                        let ones = string.charAt(string.length-1)
                        let tens = string.charAt(string.length-2)
                        let hundreds = string.charAt(string.length-3)
                        //if current score has ones, tens, and hundreds place value only
                        if (resultScore >= 100) {
                            ctx.drawImage(theme2, map[ones].imgX,map[ones].imgY,map[ones].width,map[ones].height, ( this.x + (this.w - 37) ),(this.y + (this.h/1.3 - 3)),this.wScore,this.hScore)
            
                            ctx.drawImage(theme2, map[tens].imgX,map[tens].imgY,map[tens].width,map[tens].height, ( this.x + (this.w - 37) - this.wScore ),(this.y + (this.h/1.3 - 3)),this.wScore,this.hScore)
            
                            ctx.drawImage(theme2, map[hundreds].imgX,map[hundreds].imgY,map[hundreds].width,map[hundreds].height, ( this.x + (this.w - 37) - (this.wScore*2)),(this.y + (this.h/1.3 - 3)),this.wScore,this.hScore)
        ...
        ************************/
        ...
        let draw = () => {
            ...
            pipes.render()
            ground.render()
            const resultScore = score.render()
            bird.render()
            getReady.render()
            gameOver.render(resultScore)
        }
